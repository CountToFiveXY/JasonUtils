...
public class DriverSyndicate {
    ...
    int[] coinBase = setCoinbase();
    int maxSP = setMaxSp();

    private int[] setCoinbase() {
        int c1 = 150, c2 = 375, c3 = 36300;
    ...
    private int getMaxTries(int[] coinList, int i) {
        if (coinList[i] == 0) {
            return 0;
        }
    ...
    public void getCoins(int[] coinList) {
        HashMap<Integer, int[]> record = new HashMap<>();
        int maxCoinsToStop = 0;
        for (int a = 0; a <= getMaxTries(coinList, 0); a++ ) {
            for (int b = 0; b <= getMaxTries(coinList, 1); b++ ) {
                for (int c = 0; c <= getMaxTries(coinList, 2); c++) {
                    for (int d = 0; d <= getMaxTries(coinList, 3); d++) {
                        for (int e = 0; e <= getMaxTries(coinList, 4); e++) {
                            for (int f = 0; f <= getMaxTries(coinList, 5); f++) {
                                for (int g = 0; g <= getMaxTries(coinList, 6); g++) {
                                    for (int h = 0; h <= getMaxTries(coinList, 7); h++) {
                                        for (int i = 0; i <= getMaxTries(coinList, 8); i++) {
        ...
        System.out.printf("一共可以获取 %d + %d = %d, 在 %d/%d 时停\n顺序 %s\n", maxCoinsToStop, maxCoinInList, maxCoinsToStop + maxCoinInList, stopSp, maxSP, Arrays.toString(record.get(maxCoinsToStop)));
    }

    private int[] buildCoinList (int[] reward) {
        return new int[] {
                3 * reward[0], 4 * reward[0], 5 * reward[0],
                3 * reward[1], 4 * reward[1], 5 * reward[1],
                reward[2], reward[2], reward[2]
        };
    }
